...
$('.ingredients__items .ingredient__item').hover(function(e){
  if($(window).width() > 999){
    var src = $(this).find('img.hidden').attr('src');
    var html = $(this).find('.rte_content').html();
    $('.real-ingredients .real-ingredients_left img').attr('src',src);
    $('.real-ingredients .real-ingredients_right .rte').html(html);
    $(this).closest('.ingredients__items').find('.active_text').removeClass('active_text');
    $(this).addClass('active_text')
  }
},function(e){
  if($(window).width() > 999){
    var src = $('.ingredients__items .ingredient__item:first-child').find('img.hidden').attr('src');
    var html = $('.ingredients__items .ingredient__item:first-child').find('.rte_content').html();
    $('.real-ingredients .real-ingredients_left img').attr('src',src);
    $('.real-ingredients .real-ingredients_right .rte').html(html);
    $('.ingredients__items .ingredient__item').removeClass('active_text');
    $('.ingredients__items .ingredient__item:first-child').addClass('active_text');
  }
})

$('.ingredients__items .ingredient__item').on('click',function(e){
...
$(document).on('click', function (e) {
  if($(window).width() < 1000){
    if ($(e.target).closest('.ingredient__item').length === 0) {
      var src = $(this).find('img.hidden').attr('src');
      var html = $(this).find('.rte_content').html();
      $('.real-ingredients .real-ingredients_left img').attr('src',src);
      $('.real-ingredients .real-ingredients_right .rte').html(html);
...
$('.ctm_wrapper .ctm_dropdown').on('change',function(e){
...
$('.variants-checkbox-item .variant-item').on('click',function(e){
  var val = $(this).find(':radio').val();
  var _check_subscribe;
  if($(this).closest('.variants-checkbox-item').hasClass('has-subscribe')){
    _check_subscribe = true;
  }else{
    _check_subscribe = false;
  }
  $(this).closest('.product__info').find('.product-form__option-selector .block-swatch').each(function(e){
    var curr_val = $(this).find(':radio').val();
    if(curr_val == val){
      $(this).find('label')[0].click();
      console.log(_check_subscribe, $(this).closest('.product__info').find('.rc_block__type__autodeliver label'));

      if(_check_subscribe){
        $(this).closest('.product__info').find('.rc_block__type__autodeliver label').trigger('click');
      }else{
        $(this).closest('.product__info').find('.rc_block__type__onetime label').trigger('click');
      }
    }
  })
});
...
$(document).ready(function () {
  $('.product-form__description p, .product-form__description span').click(function() {
    $('html, body').animate({
      scrollTop: $(".product-content").offset().top - 200
    }, 1000)
  });
});
